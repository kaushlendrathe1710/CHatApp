...
function sendSSEEvent(clientId: string, eventType: string, data: any): boolean {
  ...
  if (res.writableEnded) {
    console.warn(
      `[SSE] Attempted to write to ended stream for client ${clientId}`
    );
...
function broadcastPresenceUpdate() {
  const onlineUserIds = Array.from(
    new Set(Array.from(sseClients.values()).map((c) => c.userId))
  );
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/events/subscribe", isAuthenticated, async (req: any, res) => {
    ...
    if (!clientId || !conversationIds) {
      return res
        .status(400)
        .json({ error: "clientId and conversationIds required" });
    }
    ...
    const authorizedIds = conversationIds.filter((id: string) =>
      userConvIds.has(id)
    );
    const unauthorizedIds = conversationIds.filter(
      (id: string) => !userConvIds.has(id)
    );

    if (unauthorizedIds.length > 0) {
      console.warn(
        `[SSE] Client ${clientId} attempted unauthorized subscriptions:`,
        unauthorizedIds
      );
    }
    ...
    console.log(
      `[SSE] Client ${clientId} subscribed to ${authorizedIds.length} conversations`
    );
  ...
  app.get("/api/users", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const discoverableUsers = await storage.getDiscoverableUsers(
        currentUserId
      );
  ...
  const updatePrivacySchema = z.object({
    ...
    lastSeenVisibility: z
      .enum(["everyone", "connections", "hidden"])
      .optional(),
  ...
  app.put("/api/users/privacy", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (onlineStatusVisibility !== undefined)
        updates.onlineStatusVisibility = onlineStatusVisibility;
  ...
  app.get("/api/users/check-username/:username", async (req: any, res) => {
    try {
      ...
      const available =
        !existingUser || (currentUserId && existingUser.id === currentUserId);
    ...
    try {
      ...
      try {
        ...
        if (!canAccess) {
          return res
            .status(403)
            .json({ message: "Not authorized to use this object" });
        }
        ...
        return res
          .status(400)
          .json({ message: "Invalid or inaccessible object" });
      }
    } catch (error) {
    ...
    async (req: any, res) => {
      try {
        ...
        if (!canView) {
          return res
            .status(403)
            .json({ message: "Not authorized to view this user's photos" });
        }
  ...
  app.get("/api/photos/:photoId", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!canView) {
        return res
          .status(403)
          .json({ message: "Not authorized to view this photo" });
      }
    ...
    async (req: any, res) => {
      try {
        ...
        if (!canView) {
          return res
            .status(403)
            .json({ message: "Not authorized to like this photo" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (!canView) {
          return res
            .status(403)
            .json({ message: "Not authorized to unlike this photo" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (!canView) {
          return res
            .status(403)
            .json({ message: "Not authorized to view this photo's likes" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (!canView) {
          return res
            .status(403)
            .json({ message: "Not authorized to comment on this photo" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (!canView) {
          return res
            .status(403)
            .json({ message: "Not authorized to view this photo's comments" });
        }
  ...
  app.post("/api/conversations", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (currentUserRole === "user") {
        ...
        const hasRegularUser = participantUsers.some(
        ...
        if (hasRegularUser) {
          return res.status(403).json({
            message:
              "Regular users can only start conversations with admins or super admins",
          });
        }
      }
    ...
    async (req: any, res) => {
      try {
        ...
        if (!isSystemAdmin && !isGroupAdmin) {
          return res
            .status(403)
            .json({ message: "Only admins can add participants to groups" });
        }
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        if (!isSystemAdmin && !isGroupAdmin) {
          return res.status(403).json({
            message: "Only admins can remove participants from groups",
          });
        }
        ...
        if (
          ...
          return res.status(400).json({
            message:
              "Cannot remove yourself as the last admin. Promote another member to admin first.",
          });
        }
  ...
  app.patch(
    "/api/conversations/:id/participants/:participantId/role",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const { id, participantId } = req.params;
        ...
        if (!isSystemAdmin && !isGroupAdmin) {
          return res
            .status(403)
            .json({ message: "Only admins can change participant roles" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (!name || typeof name !== "string" || name.trim().length === 0) {
          return res
            .status(400)
            .json({ message: "Valid group name is required" });
        }
        ...
        if (!isSystemAdmin && !isGroupAdmin) {
          return res
            .status(403)
            .json({ message: "Only admins can rename groups" });
        }

        // Update group name
        const updatedConversation = await storage.updateConversationName(
          id,
          name.trim()
        );
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const updatedCount = await storage.markMessagesAsRead(
          conversationId,
          userId
        );
  ...
  app.post("/api/messages", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (conversation?.isBroadcast) {
        const canSend = await storage.canSendToBroadcast(
          conversationId,
          userId
        );
        if (!canSend) {
          return res
            .status(403)
            .json({ message: "Only admins can post to broadcast channels" });
        }
      }
      ...
      const clientsInConversation = Array.from(sseClients.values()).filter(
        (client) =>
          client &&
          client.conversationIds &&
          client.conversationIds.includes(conversationId) &&
    ...
    async (req: any, res) => {
      try {
        ...
        if (
          ...
          return res
            .status(400)
            .json({ message: "conversationIds array is required" });
        }

        const forwardedMessages = await storage.forwardMessage(
          messageId,
          conversationIds,
          userId
        );
    ...
    async (req: any, res) => {
      try {
        ...
        if (disappearingMessagesTimer === undefined) {
          return res
            .status(400)
            .json({ message: "disappearingMessagesTimer is required" });
        }

        const conversation = await storage.updateDisappearingMessagesTimer(
          conversationId,
        ...
        res
          .status(500)
          .json({ message: "Failed to update conversation settings" });
      }
    }
  );
  ...
  app.post("/api/broadcast/create", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const channel = await storage.createBroadcastChannel(
        name,
        description || "",
        userId
      );
  ...
  app.post(
    "/api/broadcast/:channelId/subscribe",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.id;
        ...
        res
          .status(500)
          .json({ message: "Failed to subscribe to broadcast channel" });
      }
    }
  );
  ...
  app.post(
    "/api/broadcast/:channelId/unsubscribe",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.id;
        ...
        res
          .status(500)
          .json({ message: "Failed to unsubscribe from broadcast channel" });
      }
    }
  );
  ...
  app.get("/api/webrtc/ice-servers", isAuthenticated, (req: any, res) => {
    try {
      ...
      if (turnServerUrl && turnUsername && turnPassword) {
        ...
        // Return empty config if TURN not configured
        res.json({});
      }
    } catch (error) {
  ...
  // Initiate a call
  app.post("/api/call/initiate", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      ...
      if (!conversationId || !callType) {
        return res
          .status(400)
          .json({ message: "conversationId and callType are required" });
      }

      if (!["audio", "video"].includes(callType)) {
        return res
          .status(400)
          .json({ message: "callType must be 'audio' or 'video'" });
      }
      ...
      // Broadcast call initiation to other participants (exclude caller)
      console.log(
        `[Call] User ${userId} initiating ${callType} call in conversation ${conversationId}`
      );
      for (const [clientId, client] of sseClients.entries()) {
        if (
          client.conversationIds.includes(conversationId) &&
          ...
          console.log(
            `[Call] Sending call_initiate to client ${clientId} (user ${client.userId})`
          );
          sendSSEEvent(clientId, "call_initiate", {
            conversationId,
            callType,
            callerId: userId,
  ...
  app.post("/api/call/signal", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const { conversationId, signal } = req.body;

      if (!conversationId || !signal) {
        return res
          .status(400)
          .json({ message: "conversationId and signal are required" });
      }
      ...
      // Broadcast signaling data to other participants (exclude sender)
      console.log(
        `[Call] User ${userId} sending ${signal.type} signal to conversation ${conversationId}`
      );
      for (const [clientId, client] of sseClients.entries()) {
        if (
          client.conversationIds.includes(conversationId) &&
          ...
          console.log(
            `[Call] Forwarding ${signal.type} signal to client ${clientId} (user ${client.userId})`
          );
          sendSSEEvent(clientId, "call_signal", {
            conversationId,
            signal,
            senderId: userId,
          });
  ...
  // End a call
  app.post("/api/call/end", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
  ...
  // Reject a call
  app.post("/api/call/reject", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      ...
      // Broadcast call rejection to other participants
      broadcastToConversation(conversationId, {
        type: "call_rejected",
        data: {
          conversationId,
  ...
  app.post("/api/encryption/keys", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!conversationId || !publicKey) {
        return res
          .status(400)
          .json({ message: "conversationId and publicKey are required" });
      }

      const key = await storage.storeEncryptionKey({
        conversationId,
        userId,
        publicKey,
      });
