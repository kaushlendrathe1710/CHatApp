...
function ChatMessageComponent({
  ...
  const renderForwardedFromBadge = () => {
    ...
    return (
      <div
        ...
        data-testid={`text-forwarded-from-${message.id}`}
      >
  ...
  const renderReplyPreview = () => {
    ...
    const repliedContent = getReplyPreviewText(
  ...
  const renderMessageContent = () => {
    ...
    if (message.isEncrypted) {
      if (decryptionError) {
        return (
          <div
            ...
            data-testid={`text-decryption-error-${message.id}`}
          >
      ...
      if (!decryptedContent) {
        return (
          <div
            ...
            data-testid={`text-decrypting-${message.id}`}
          >
      ...
      return (
        <div className="space-y-1">
          ...
          >
            {parseLinksInText(decryptedContent)}
          </p>
        </div>
      );
    }
    ...
    return (
      <p
        ...
        data-testid={`text-message-${message.id}`}
      >
        {message?.content && parseLinksInText(message.content)}
      </p>
    );
  };
  ...
  return (
    <div
      ...
      } items-end ${isSelectionMode ? "cursor-pointer" : ""}`}
    ...
    >
      ...
      <div
        ...
        } max-w-[65%] md:max-w-[50%] min-w-0 gap-1`}
      >
        {isGroup && !isOwn && showAvatar && (
          <span
            className="text-xs font-medium px-3 text-muted-foreground truncate max-w-full"
            data-testid="text-sender-name"
          >
            {senderName}
          </span>
        )}
            ...
            <div className={`flex flex-col gap-0.5 mt-1`}>
              ...
              >
                {message.isEdited && (
                  <span
                    ...
                    data-testid="text-edited-indicator"
                  >
                    edited
                  </span>
                )}
              ...
              {getExpirationText() && (
                ...
                >
                  ...
                  <span
                    className="text-xs"
                    data-testid={`text-expiration-${message.id}`}
                  >
                    {getExpirationText()}
                  </span>
                </div>
