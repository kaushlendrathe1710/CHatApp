...
- **Username System:** Unique usernames (alphanumeric, 3-20 characters) required for all users. Features include: (1) Real-time availability checking during registration and profile updates, (2) Visual feedback (green checkmark for available, red X for taken), (3) Username-based search functionality on People page, (4) Display usernames throughout the app instead of emails for better privacy.
- **User Search & Discovery:** Telegram/Instagram-style username search that allows users to find and message each other. Features include: (1) Live search endpoint (`GET /api/users/search`) with debounced queries (300ms), (2) Search by username with case-insensitive matching, (3) Respects all privacy settings - users with "hidden" profiles never appear, users with "past_chats" visibility only appear to people they've previously messaged, (4) Results limited to 20 users for performance, (5) One-click "Start Chat" to begin conversations with found users, (6) Role-based visibility: regular users see empty state by default and must search to find people (prevents browsing all users), while admins/super_admins can see all users by default for moderation purposes.
...
- **Users:** `/api/users`, `/api/users/search` (username search), `/api/users/discoverable`, `/api/users/:userId/can-view`, `/api/users/privacy`, `/api/users/check-username/:username` (availability check), `/api/users/profile` (update full profile)
