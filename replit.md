...
- **Real-time Features:** Automatic conversation list updates via 10-second polling (TanStack Query `refetchInterval`). WebSocket infrastructure present but non-functional due to Replit proxy blocking upgrade requests. In-conversation messaging uses WebSocket for typing indicators and message delivery where WebSocket connections succeed.
...
- **Group Chat:** Admin-only group creation and management. Features include: (1) Create groups with optional initial participants (admins can create groups with just themselves), (2) Group settings dialog for participant management (add/remove/promote to admin), (3) Admin-only controls - group settings button visible only to system admins or group admins, (4) All individual chat features work in groups (read receipts, typing indicators, message actions), (5) Backend enforces access control: only admins can create groups, only system admins or group admins can manage participants, (6) Prevention of orphaned groups - cannot remove last admin without promoting another member first.
