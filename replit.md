...
- **Notification Sounds (Production-Ready):** Customizable audio notifications for incoming messages with comprehensive security and state management. Architecture includes: (1) NotificationSoundManager singleton with hydration guard system - `reset()` clears state on auth change, `hydrate(settings)` enables SSE playback with validated settings, `primeLocal(settings)` updates UI without enabling playback (for optimistic updates), `play()` gated by hydration flag, (2) AuthTransitionObserver component mounted globally in App.tsx watches auth query changes and calls `manager.reset()` + removes all cache variants (`exact: false`) on user change (runs on ALL routes), (3) Per-user settings stored in database (notificationSoundEnabled, notificationSoundType, notificationVolume) with defaults, (4) Backend API endpoints GET/PUT `/api/users/notification-sound` return userId field for validation, (5) Local bundled audio files (5 sounds) in attached_assets/sounds/, (6) Settings UI in Privacy Settings page with all controls having data-testid attributes, (7) React Query with user-scoped cache keys `['/api/users/notification-sound', user?.id]`, (8) Home.tsx and PrivacySettings.tsx call `manager.hydrate()` only after validating `response.userId === user.id`, (9) Optimistic updates use `manager.primeLocal()` to update UI without enabling SSE playback, (10) SSE playback blocked by `!hydrated` guard until validated settings arrive, (11) Test sounds use separate uncached Audio instances, (12) Zero React hook errors, zero cross-user preference leakage.
- **Mobile Viewport Optimization:** Cross-platform solution for handling mobile browser chrome using `useViewportSafeArea` custom hook. Combines iOS safe-area-inset env vars with Android visualViewport API to dynamically track browser UI (status bar, address bar) position. Main layout uses `fixed inset-0 overflow-hidden` with calculated padding (`max(16px, calc(env(safe-area-inset-top) + var(--viewport-safe-top)))`) to ensure header and composer remain visible on both iOS (with notch support) and Android (with collapsing address bar). Viewport meta includes `viewport-fit=cover` for proper edge-to-edge display. Camera button hidden on mobile (accessible via attachment menu) to maintain 44×44px minimum touch targets for all visible composer controls. Top bar and sidebar completely fixed (non-scrollable) - only conversation lists and messages scroll.
...
- **Users:** `/api/users`, `/api/users/discoverable`, `/api/users/:userId/can-view`, `/api/users/privacy`, `/api/users/notification-sound` (GET/PUT - manage notification sound settings)
- **Conversations:** `/api/conversations`, `/api/conversations/:id`, `/api/conversations/:id/settings`, `/api/conversations/:id/name` (PATCH - rename group)
